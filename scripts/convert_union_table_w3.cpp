...
void convert_union_file(int num, std::map<std::string,
    ...
    for (; i != inverted_MM.end(); i++) {
        ...
        for (; j != inverted_MM.end(); j++) {
            if (num <= 0) break;
            std::unordered_set<std::string> k2(j->second.begin(), j->second.end());
            auto k = j;
            ++k;
            for (; k != inverted_MM.end(); k++) {
                if (num <= 0) break;
                bool flag = false;
                for (const auto &z: k->second) {
                    if (k1.find(z) != k1.end() && k2.find(z) != k2.end()) {
                        if (!flag) {
                            os << i->first << "∩" << j->first << "∩" << k->first << " ";
                            flag = true;
                        }
                        os << z << " ";
                        --num;
